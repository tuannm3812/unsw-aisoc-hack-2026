...
function TaskInspector({ node }: { node: GraphNode }) {
  ...
  const [alignResult, setAlignResult] = useState<{ conflicts: { nodeA: string; nodeB: string; description: string }[]; summary: string } | null>(null)
  const [activeTab, setActiveTab] = useState("details")
  const [breakingDown, setBreakingDown] = useState(false)
  const [subtasks, setSubtasks] = useState<{ title: string; body: string }[] | null>(null)
  ...
  async function checkAlignment() {
    ...
    try {
      // ponytail: demo mock — wire to /api/check-alignment when backend supports it
      await new Promise((resolve) => setTimeout(resolve, 900))
      setAlignResult({
        summary: "1 conflict found between connected context nodes",
        conflicts: [
          {
            nodeA: "Meeting Notes (Jul 28)",
            nodeB: "Design Decision Log",
            description: "Meeting Notes specify using email OTP for auth, but the Design Decision Log chooses magic links with 'no passwords, no OTP codes.' These approaches are incompatible.",
          },
        ],
      })
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex min-h-0 flex-1 flex-col">
      <div className="border-border border-b px-5 pt-3">
        <TabsList className="h-9 bg-transparent p-0">
          ...
          <TabsTrigger value="alignment" className="h-8 rounded-lg px-3 text-xs">
            Alignment
          </TabsTrigger>
        </TabsList>
      </div>
      ...
      >
        ...
        <section>
          ...
          >
            {breakingDown && <Loader2 className="size-3.5 animate-spin" />}
            Break into subtasks
          </Button>
          <p className="text-muted-foreground mt-2 text-xs leading-relaxed">
            Mistral reads the lineage and suggests concrete subtasks. Created tasks land on the canvas.
          </p>
          {subtasks && (
            <div className="mt-3 space-y-1.5">
              {subtasks.map((st, i) => (
                <div key={i} className="bg-accent/20 rounded-lg px-3 py-2 text-xs">
                  <p className="font-medium">{st.title}</p>
                  <p className="text-muted-foreground mt-0.5 text-2xs">{st.body}</p>
                </div>
              ))}
            </div>
          )}
          ...
          alignResult.conflicts.length > 0 ? (
            <div className="space-y-4">
              <div className="rounded-lg border border-warning/40 bg-warning/5 p-3">
                <p className="text-sm font-medium">{alignResult.summary}</p>
              </div>
              {alignResult.conflicts.map((c, i) => (
                <div key={i} className="rounded-lg border p-3">
                  <div className="mb-2 flex items-center gap-2 text-xs">
                    <span className="rounded-full bg-accent px-2 py-0.5 font-mono font-semibold">
                      {c.nodeA}
                    </span>
            ...
            <div className="py-12 text-center">
              <Check className="text-success mx-auto mb-2 size-8" />
              <p className="text-sm font-medium">No conflicts detected</p>
              <p className="text-muted-foreground mt-1 text-xs">
                All connected context nodes appear aligned
              </p>
            </div>
          )
        ) : (
